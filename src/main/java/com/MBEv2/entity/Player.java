...
import com.MBEv2.entity.entities.TNT_Entity;
...
import com.MBEv2.core.Launcher;
import org.joml.*;
...
import static com.MBEv2.generation.WorldGeneration.WATER_LEVEL;
...
//            }
    ...
    private void fillVisibleChunks(int chunkX, int chunkY, int chunkZ, int entrySide, int traveledDirections, int damper) {
        ...
        if (Chunk.readOcclusionCullingSidePair(entrySide, NORTH, occlusionCullingData) && (traveledDirections & 1 << SOUTH) == 0)
            fillVisibleChunks(chunkX, chunkY, chunkZ + 1, SOUTH, traveledDirections | 1 << NORTH, damper);
        if (Chunk.readOcclusionCullingSidePair(entrySide, SOUTH, occlusionCullingData) && (traveledDirections & 1 << NORTH) == 0)
        ...
        if (Chunk.readOcclusionCullingSidePair(entrySide, WEST, occlusionCullingData) && (traveledDirections & 1 << EAST) == 0)
            fillVisibleChunks(chunkX + 1, chunkY, chunkZ, EAST, traveledDirections | 1 << WEST, damper);
        if (Chunk.readOcclusionCullingSidePair(entrySide, EAST, occlusionCullingData) && (traveledDirections & 1 << WEST) == 0)
